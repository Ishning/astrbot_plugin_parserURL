...
class WeiBoParser(BaseParser):
    ...
    async def parse_weibo_id(self, weibo_id: str):
        ...
        for attempt in range(max_retries):
            try:
                async with self.session.get(
                    ...
                    if resp.status != 200:
                        if resp.status in (403, 418):
                            raise ParseException(f"被风控拦截（{resp.status}）")
                        raise ParseException(f"获取失败 {resp.status}")

                    ctype = resp.headers.get("content-type", "")
                    if "application/json" not in ctype:
                        raise ParseException(f"响应格式错误: {ctype}")
                ...
                if attempt == max_retries - 1:
                    raise ParseException(f"网络异常，已重试{max_retries}次: {e}")
                
                wait_time = (attempt + 1) * 2
                print(f"连接断开或超时，{wait_time}秒后进行第{attempt+2}次重试...")
        ...
        try:
            #反序列化
            result = msgspec.json.decode(body_bytes, type=WeiboResponse)
            weibo_data = result.data
